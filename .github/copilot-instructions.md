...
- **linkedin-official-publisher**: Drafts, formats, and publishes high-impact posts to LinkedIn using the official REST API (/rest/posts) with mandatory versioning headers, automatic UUID idempotency keys, and interactive Rich terminal previews with dry-run confirmation before live transmission. Trigger with `/linkedin-publish [topic_or_draft]`.
- **linkedin-company-manager**: Monitors LinkedIn Organization Pages, inspects recent company updates, retrieves inbound comment threads, and posts official company replies using LinkedIn's Community Management API (/rest/socialActions). Trigger with `/linkedin-company [comments|posts|reply]`.
