...
name: code-reviewer
description: >-
  Performs deep, senior-level code reviews focusing on architecture, security vulnerabilities,
  performance bottlenecks, test coverage, and adherence to clean code principles.
  Trigger with `/review [path_or_git_diff]`.
parameters:
  target:
    type: string
    description: File path, directory, or git commit/branch to review
    required: false
    default: "HEAD"
---
